	...
	public class FMovePlayerSystem : IEcsRunSystem 
	{
		...
		public void Run()
		{
			for (int i = 0; i < _playerFilter.EntitiesCount; i++)
			{
				...
				float speedForce = (Mathf.Abs(x) + Mathf.Abs(y)) / 2;
				player.Transform.Translate(speedVector * speedForce * SpeedMultipiler);
				//
				if (_collisionFilter != null)
				{
					for (int j = 0; j < _collisionFilter.EntitiesCount; j++)
					{
						_world.RemoveComponent<CollisionEvent>(_collisionFilter.Entities[j]);

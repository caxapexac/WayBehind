...
namespace Misc
{
    public static class MapGenSquareFractalNoise
    {
        public static HexaList3D<HexComponent> GenerateMap(int saeed, int octaves, float persistance, int radius, int depth, float hexSize)
        {
            ...
            float[,] noise = FractalNoise.Get((int)(Random.value * 10000), radius * 2, octaves, persistance);
            for (int i = -radius; i < radius; i++)
            {
                for (int k = -radius; k < radius; k++)
                {
                    ...
                    {
                        //map[i, k, 0] = new HexComponent(){ HexType = HexTypes.Grass, Color = noise[i + radius, k + radius]};
                        Vector2 hexPos = HexMath.Hexel2Pixel(i + radius, k + radius, hexSize);
                        //float height = noise[Mathf.RoundToInt(hexPos.x), Mathf.RoundToInt(hexPos.y)];
